...
func (w *Worker) runDocumentReminderJob(ctx context.Context) error {
	type row struct {
	...
	err := w.store.DB.SelectContext(ctx, &rows, `
		...
		WHERE n.type = 'thing'
		  ...
		  AND i.is_document = true
		  AND i.expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + 90
	`)
	if err != nil {
		return err
	}
	...
	for _, item := range rows {
		title := fmt.Sprintf("证件到期提醒：%s", item.Name)
		body := fmt.Sprintf("证件 %s 将在 %d 天内到期，请提前办理", item.Name, item.DaysRemaining)
		_, _ = w.store.DB.ExecContext(ctx, `
			INSERT INTO reminders (user_id, node_id, title, remind_at, source)
			SELECT $1, $2, $3, $4, 'item-document'
			WHERE NOT EXISTS (
				SELECT 1 FROM reminders
				WHERE user_id = $1 AND node_id = $2 AND source = 'item-document' AND is_done = false
			)
